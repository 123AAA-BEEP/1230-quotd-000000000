...
// (Project → Settings → Git) with Root Directory `sites/gtahelicalpiles` so
...
const name = process.env.PROJECT_NAME || 'gtahelicalpiles';
...
try {
  ...
  project = await api('/v11/projects', {
    method: 'POST',
    body: JSON.stringify({ name, framework: 'astro', rootDirectory: 'sites/gtahelicalpiles' }),
  });
